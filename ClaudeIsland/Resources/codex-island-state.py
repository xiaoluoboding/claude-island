...
"""
CLI Island Hook
- Sends Codex/Copilot session state to ClaudeIsland.app via Unix socket
"""
import json
import os
import select
...
def parse_args(argv):
    ...
    for i, token in enumerate(argv):
        if token == "--source" and i + 1 < len(argv):
            source = argv[i + 1].strip().lower() or "codex"
